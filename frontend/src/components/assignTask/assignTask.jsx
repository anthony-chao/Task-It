...
const AssignTask = (props) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!handleError()) {
            let selectedUser = props.allUsers.map((object) => [object._id, object.email]).filter((arr) => arr[1] === input.email.split("(")[1].slice(0, -1));
            ...
            <div>
                { (error.error) ? 
                    <Stack sx={{ width: '100%' }} spacing={2}>
                        <Alert variant="outlined" severity="error" onClose={() => {setError({open: false})}}>You must enter a valid user and task!</Alert>
                    </Stack>
                : null }
                { (error.success) ? 
                    <Stack sx={{ width: '100%' }} spacing={2}>
                        <Alert variant="outlined" severity="success" onClose={() => {setError({open: false})}}>You have successfully assigned {input.email.split(" ")[0]} a task!</Alert>
                    </Stack>
                : null }
                <form onSubmit={handleSubmit}>
                    ...
                    <Autocomplete
                        id="free-solo-demo"
                        freeSolo
                        options={props.allUsers.map((option) => option.firstName.concat(" ", option.lastName, " (", option.email, ")"))}
...
const mapDispatchToProps = dispatch => ({
    ...
    updateAssignedTask: (taskId, userId) => dispatch(updateAssignedTask(taskId, userId))
})
